...
const PlayerIcon = (props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const leagueRes = await fetchProxied(
        `https://ch.tetr.io/api/users/${props.username.toLowerCase()}/summaries/league`
      );
      const userRes = await fetchProxied(
        `https://ch.tetr.io/api/users/${props.username.toLowerCase()}`
      );
      ...
      if (user.avatar_revision) {
        ...
        setAvatarUrl(
          `data:image/svg+xml;base64,${new window.Identicon(window.MD5(user._id), {
    ...
    <div className="player-icon" style={style}>
      {avatarUrl ? (
        <img
          ...
          alt="icon"
          src={avatarUrl}
          style={{ borderColor: teamColor, opacity: props.eliminated ? 0.3 : 1 }}
        />
      ) : (
      ...
      {banned && (
        <img
          ...
          src={`${import.meta.env.BASE_URL}Banned_Icon.png`}
          alt="Banned"
        />
      )}
      <div className="label" style={{ opacity: props.eliminated ? 0.6 : 1 }}>
        <svg width="200%" height="200%" viewBox="0 0 1000 75" preserveAspectRatio="xMinYMid meet">
          <text
            x="50%"
            y="50%"
